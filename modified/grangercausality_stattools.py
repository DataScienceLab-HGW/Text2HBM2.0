'''
This script contains a modified version of the implementation of the 
granger causality calculation from the package stattools
The original script needed to be modified in order to 
find a workaround for an exception occuring in the original package
which checks if calculating the granger causality is feasable or not

Mod 1:
the value 

Mod 2: 
If the value of the input vector is constant, the package throws an exception:
"The x values include a column with constant values and so the test statistic cannot be computed."

the original python package has a BSD-3 Clause license https://opensource.org/licenses/BSD-3-Clause
For license, see the file LICENSE_statsmodels.txt

Original code: 
https://github.com/statsmodels/statsmodels/blob/main/statsmodels/tsa/stattools.py
...
from statsmodels.tools.validation.validation import array_like, bool_like, int_like
from statsmodels.tsa.tsatools import lagmat2ds
from statsmodels.tools import add_constant

def grangercausalitytests(x, maxlag, addconst=True, verbose=False):
    """
    ...
    for mlg in lags:
        ...
        if (
                ...
                or (res2djoint.ssr / tss) == 0 # the original check is < np.finfo(float).eps
                # which means that it checks to a certain precision 
